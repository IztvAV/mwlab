...
__all__ = [
    "Device",
    "Filter",
]  # Multiplexer намеренно не экспортируется


# ────────────────────────────────────────────────────────────────────────────
#                                 DEMO
# ────────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":                  # быстрый smoke-тест
    import matplotlib.pyplot as plt
    from mwlab.filters.topologies import get_topology

    topo = get_topology("folded", order=4)
    cm   = CouplingMatrix(topo,
                          M_vals={"M1_2":1.0, "M2_3":0.9, "M3_4":1.0, "M1_4":0.2},
                          Q=[7_000]*4)
    ...
    plt.plot(f/1e9, 20*np.log10(np.abs(S[:,0,1])))
    plt.title(str(flt)); plt.xlabel("freq, GHz"); plt.ylabel("|S21|, dB")
    plt.grid(); plt.show(block=False); plt.pause(2); plt.close()

    print("Ω → f (GHz):", flt.freq_grid([-2, -1, 0, 1, 2], unit="GHz"))

    ts        = flt.to_touchstone(f, unit="Hz")
    restored  = Filter.from_touchstone(ts)
    print("Restored equals original:", restored.cm.to_dict() == flt.cm.to_dict())

# ────────────────────────────────────────────────────────────────────────────
#                                 __all__
# ────────────────────────────────────────────────────────────────────────────
__all__ = ["Device", "Filter"]
