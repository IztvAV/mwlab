...
Мини-DEMO + self-test:
    ...
    python -m mwlab.filters.cm
"""
import numpy as np

try:
    import torch
except ModuleNotFoundError:            # Torch не установлен → только NumPy-ветка
    torch = None                       # type: ignore[assignment]

from mwlab.filters.topologies import get_topology

# ── 1. Топология «folded» на 4 резонатора ───────────────────────────────────
topo = get_topology("folded", order=4, name="folded-demo")

cm = CouplingMatrix(
    topo,
    #              ┌─ цепочка ─┐  ┌─ «угловая» ┐  ┌─ портовые ─┐
    M_vals={
        "M1_2": 1.00,
        "M2_3": 0.90,
        "M3_4": 1.00,
    ...
    Q=[7000, 7200, 7100, 7300],
    phase_a={1: 0.8e-12, 2: 0.8e-12},
)

# ── 2. NumPy (CPU) расчёт ───────────────────────────────────────────────────
omega = np.linspace(-3.0, 3.0, 801, dtype=np.float32)
S_np  = cm.sparams(omega, backend="numpy")          # (F, 2, 2)

peak_np_db = 20.0 * np.log10(np.abs(S_np[:, 0, 1]).max())
print(f"NumPy  |S21|_max = {peak_np_db:6.2f} dB")

# ── 3. Torch (GPU, если доступен) ───────────────────────────────────────────
if torch is not None:
    torch.set_default_dtype(torch.float32)

    device  = "cuda" if torch.cuda.is_available() else "cpu"
    omega_t = torch.linspace(-3.0, 3.0, 801, device=device)

    S_t = cm.sparams(omega_t, backend="torch", device=device)   # (F,2,2)
    peak_t_db = 20.0 * torch.log10(torch.abs(S_t[:, 0, 1])).max().cpu().item()
    print(f"Torch  |S21|_max = {peak_t_db:6.2f} dB  (device={device})")

else:
    print("Torch не установлен — пропускаем GPU/CPU-ветку")

# ── 4. Быстрая проверка сериализации ----------------------------------------
snap = cm.to_dict()
print("\nDict-snapshot:", {k: snap[k] for k in list(snap)})